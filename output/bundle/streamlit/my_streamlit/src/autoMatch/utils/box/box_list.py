...
from .exceptions import BoxError, BoxTypeError
...
class BoxList(list):
    ...
    def __new__(cls, *args, **kwargs):
        ...
        obj.box_options = {"box_class": Box}
        obj.box_options.update(kwargs)
        obj.box_org_ref = None
        return obj

    def __init__(self, iterable: Optional[Iterable] = None, box_class: Type = None, **box_options):
    ...
    def __setitem__(self, key, value):
        ...
        if self.box_options.get("box_dots") and isinstance(key, str) and key.startswith("["):
            ...
            if self.box_options.get("default_box"):
                if children[0] == "[":
                    super().__setitem__(pos, BoxList(**self.box_options))
                else:
