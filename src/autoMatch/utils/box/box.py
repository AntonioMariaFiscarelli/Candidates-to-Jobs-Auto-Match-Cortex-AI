...
class Box(dict):
    ...
    def __get_default(self, item, attr=False):
        ...
        if default_value in (self._box_config["box_class"], dict):
            ...
            value = BoxList(**self.__box_config(extra_namespace=item))
    ...
    def __recast(self, item, value):
        if self._box_config["box_recast"] and item in self._box_config["box_recast"]:
            ...
            try:
                if isinstance(recast, type) and issubclass(recast, (Box, BoxList)):
    ...
    def __convert_and_store(self, item, value):
        ...
        if isinstance(value, dict):
            ...
            if self._box_config["frozen_box"]:
                ...
                value = BoxList(value, **self.__box_config(extra_namespace=item))
        elif isinstance(value, BoxList):
    ...
    def __setitem__(self, key, value):
        ...
        if self._box_config["box_dots"] and isinstance(key, str) and ("." in key or "[" in key):
            ...
            if first_item in self.keys():
                ...
                if children[0] == "[":
                    super().__setitem__(first_item, BoxList(**self.__box_config(extra_namespace=first_item)))
                else:
                    super().__setitem__(
                        first_item, self._box_config["box_class"](**self.__box_config(extra_namespace=first_item))
                    )
    ...
    def merge_update(self, *args, **kwargs):
        ...
        def convert_and_set(k, v):
            ...
            if isinstance(v, list) and not intact_type:
                v = BoxList(v, **self.__box_config(extra_namespace=k))
    ...
    def setdefault(self, item, default=None):
        ...
        if isinstance(default, list):
            default = BoxList(default, **self.__box_config(extra_namespace=item))
        self[item] = default
        return self[item]
